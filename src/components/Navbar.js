  ...
  return (
    <div className="flex w-full justify-between items-center h-20 px-4 absolute z-10 text-white">
      ...
      <div className="hidden md:flex">
        <BiSearch className="mr-2" shize={20} />
        <BsPerson shize={20} />
      </div>
      {/* Hamburger */}
      ...
      <div onClick={handleNav} className="md:hidden cursor-pointer z-10 ">
        {nav ? (
          ...
          <HiOutlineMenuAlt4 shize={20} />
        )}
      </div>
      {/* Mobal menue dropdown */}
      <div
        className={
          nav
            ? " md:hidden absolute text-black left-[0] top-0 w-full bg-gray-100/90 px-4 py-7 flex flex-col"
